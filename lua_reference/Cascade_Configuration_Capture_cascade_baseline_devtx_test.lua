--[[
Cascade_Configuration_Capture_cascade_baseline_devtx_test.lua

mmWave Studio capture matching radar_configs/cascade_baseline_devtx_test.toml.

Dev4 (slave3) has ALL TX off but stays RX-enabled. Chirps that used to be
Dev4-only are REMOVED (no silent no-TX slots); remaining 9 chirps remapped
like cascade_baseline_devrx_test. Isolates "Dev4 TX off + fewer chirps" from
the earlier 12-chirp silent-slot schedule that dropped heavily.

Pair with CLI:
  python3 mimo.py --radar-config cascade_baseline_devtx_test --frames 300 \
      --exp-name cli_cascade_baseline_devtx_test

GEOMETRY:
  Dev1-4 all RX (16 RX); Dev4 TX gated off
  chirp 0-2  -> Dev3 TX2/TX1/TX0
  chirp 3-5  -> Dev2 TX2/TX1/TX0
  chirp 6-8  -> Dev1 TX2/TX1/TX0
  -> deviceMapOverall = 15; all 4 devices' .bin files present
...
dev_list          =    {1, 2, 4, 8}
RadarDevice       =    {1, 1, 1, 1}       -- all 4 enabled (Dev4 RX-only)
...
devMap_Dev4       =   dev_list[4]   -- 8

WriteToLog(string.format(
    "deviceMapOverall=%d (expect 15), deviceMapSlaves=%d (expect 14)\n",
...
local end_chirp_tx              =   8        -- 9 chirps (no silent slots)
...
WriteToLog("Setting up Studio for Cascade (cascade_baseline_devtx_test)...\n", "blue")
...
if not chirp_ok(devMap_Dev4, 0, 0, 0, 0, "Chirp 0 Dev4 TX off") then return -4 end
-- Chirp 1: Dev3 TX1
...
if not chirp_ok(devMap_Dev4, 1, 0, 0, 0, "Chirp 1 Dev4 TX off") then return -4 end
-- Chirp 2: Dev3 TX0
...
if not chirp_ok(devMap_Dev4, 2, 0, 0, 0, "Chirp 2 Dev4 TX off") then return -4 end
-- Chirp 3: Dev2 TX2
...
if not chirp_ok(devMap_Dev4, 3, 0, 0, 0, "Chirp 3 Dev4 TX off") then return -4 end
-- Chirp 4: Dev2 TX1
...
if not chirp_ok(devMap_Dev4, 4, 0, 0, 0, "Chirp 4 Dev4 TX off") then return -4 end
-- Chirp 5: Dev2 TX0
...
if not chirp_ok(devMap_Dev4, 5, 0, 0, 0, "Chirp 5 Dev4 TX off") then return -4 end
-- Chirp 6: Dev1 TX2
...
if not chirp_ok(devMap_Dev4, 6, 0, 0, 0, "Chirp 6 Dev4 TX off") then return -4 end
-- Chirp 7: Dev1 TX1
...
if not chirp_ok(devMap_Dev4, 7, 0, 0, 0, "Chirp 7 Dev4 TX off") then return -4 end
-- Chirp 8: Dev1 TX0
...
if not chirp_ok(devMap_Dev4, 8, 0, 0, 0, "Chirp 8 Dev4 TX off") then return -4 end
...
capture_directory         =   "studio_cascade_baseline_devtx_test_" .. timestamp
...
WriteToLog("Expect all 4 devices (Dev4 RX-only, 9 chirps, no silent TX slots).\n", "blue")
