--[[
Cascade_Configuration_Capture_cascade_tx5_rx16_5rps.lua

mmWave Studio capture matching radar_configs/cascade_tx5_rx16_5rps.toml.

Built from Cascade_Configuration_Capture_cascade_tx5_rx16_robotic.lua's
chirp geometry (same txAntennaTable/profileIdPerChirp as the TOML), with
RF/frame timing swapped back to the 5rps spinning-schedule values (255
loops/frame, 65ms period) instead of the robotic preset's SAR
loop-per-frame timing.
...
TX12 - matches the TOML's txAntennaTable):
  ...
  THREE devices (Dev2, Dev3, Dev4) each transmit on a subset of the 5
  chirps - only Dev1 (master) is RX-only on every chirp.

RF / FRAME TIMING (matches cascade_tx5_rx16_5rps.toml exactly):
  5 chirps, 3 profiles reused via profileIdPerChirp = [0,2,1,0,2], idle
  7/7/7 us, ramp 40 us, slope 95 MHz/us, 160 samples @ 4850 ksps, rxGain
  48, numLoops 255, framePeriodicity 65 ms (the "5rps" name).

DATA RATE: 160 x 5 x 255 x 16 RX x 4 bytes / 0.065s ~= 201 MB/s. This
config is EXPECTED to show mid-capture frame drops under the CLI
(mimo.py) - the point of this script is to check whether the same drop
pattern shows up when armed from Studio instead, at the same nominal RF
config (see mimo.py's FRAME DROPS docstring; prefer
radar_configs/cascade_baseline.toml for a known-clean reference).

DEFAULT CAPTURE LENGTH: 200 frames (~13 s @ 65 ms). Edit
nframes_master/slave below if you want a shorter/longer run.

AFTER THE CAPTURE:
  python3 parse_idx.py --fetch <capture_directory> --frames 200 --period-ms 65
...
-- radar_configs/cascade_tx5_rx16_5rps.toml's txAntennaTable exactly)
...
-- two), so the off-group differs by chirp:
...
-- Per-profile idle times (matches cascade_tx5_rx16_5rps.toml)
...
local nchirp_loops              =   255      -- matches TOML numLoops
local nframes_master            =   200      -- edit for a shorter/longer run
local nframes_slave             =   200
local Inter_Frame_Interval      =   65       -- ms - matches TOML framePeriodicity ("5rps")
local trigger_delay             =   0        -- us

------------------------------ API Configuration ------------------------------------------------

WriteToLog("Setting up Studio for Cascade (cascade_tx5_rx16_5rps)...\n", "blue")
...
-- Chirp geometry matches cascade_tx5_rx16_5rps.toml's txAntennaTable
...
capture_directory         =   "studio_tx5_rx16_5rps_" .. timestamp
