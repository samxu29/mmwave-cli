...
This MRR version obtains the frame count and frame interval from
Cascade_Configuration_MIMO_MRR.lua and calculates capture_time automatically.
Run Cascade_Configuration_MIMO_MRR.lua successfully before running this script.
--]]

capture_margin_time              =   5000                             -- ms
inter_loop_time                  =   2000                             -- ms
num_loops                        =   1

if ((MRR_CAPTURE_NUM_FRAMES == nil) or (MRR_CAPTURE_FRAME_INTERVAL_MS == nil)) then
    WriteToLog("MRR Capture Error: Run Cascade_Configuration_MIMO_MRR.lua successfully before this script.\n", "red")
    return -5
end

capture_time = math.ceil(MRR_CAPTURE_NUM_FRAMES * MRR_CAPTURE_FRAME_INTERVAL_MS + capture_margin_time)

WriteToLog("MRR Capture: configured frames = "..MRR_CAPTURE_NUM_FRAMES.."\n", "blue")
WriteToLog("MRR Capture: frame interval = "..MRR_CAPTURE_FRAME_INTERVAL_MS.." ms\n", "blue")
WriteToLog("MRR Capture: automatic wait time = "..capture_time.." ms\n", "blue")
...
capture_directory               =   "0822_MRR_SanityCheck02_" .. timestamp
n_files_allocation              =   0
data_packaging                  =   0             -- 0: 16-bit, 1: 12-bit
num_frames_to_capture           =   0                       -- Keep 0; frame count comes from radar frame configuration
