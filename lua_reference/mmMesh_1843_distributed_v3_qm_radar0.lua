...
-- Modified: Split IDLE_TIME into Fast (for clustering) and Delay (for end of loop)
IDLE_TIME_FAST = 7
IDLE_TIME_DELAY = 254
RAMP_END_TIME = 65
CHIRP_LOOPS = 255
...
autoStart = 1
-- Modified: Changed to 2 profiles
NumOfProfile = 2
NumOfChirpInLoop = 2
NumOfFrame = 0

profile = {}
-- Modified: Profile 0 uses the fast idle time
profile[0] = { 0, START_FREQ, IDLE_TIME_FAST, ADC_START_TIME, RAMP_END_TIME, 0, 0, 0, 0, 0, 0, FREQ_SLOPE, 0, ADC_SAMPLES,
    SAMPLE_RATE, 0, 0, RX_GAIN }
-- Modified: Profile 1 uses the delayed idle time
profile[1] = { 1, START_FREQ, IDLE_TIME_DELAY, ADC_START_TIME, RAMP_END_TIME, 0, 0, 0, 0, 0, 0, FREQ_SLOPE, 0,
...
if (partId == 1642) then
    ...
    if (ar1.LPModConfig(0, 0) == 0) then
        WriteToLog("Regular mode Cfg Success\n", "green")
    else
        WriteToLog("Regular mode Cfg failure\n", "red")
    end
...
-- Modified: Switch to delayed profile for the 3rd chirp
for i = 0, NumOfChirpInLoop - 1 do
    if i == 0 then
        p = 1 -- Use Profile 1 (delayed) for the 3rd chirp
    else
        p = 0 -- Use Profile 0 (fast) for the 1st and 2nd chirps
    end
...
-- power off
-- switch 2.6 to GND (ON)
-- power on
-- ar1.SelectCaptureDevice("DCA1000")
-- ar1.CaptureCardConfig_EthInit("192.168.33.30", "192.168.33.180", "12:34:56:78:90:12", 4096, 4098)
-- ar1.ConfigureRFDCCard_EEPROM("[custom_PC_IP]", "[custom_DCA_IP]", "[custom_DCA_MAC]", 4096, 4098)
-- power off
-- siwtch 2.6 to pin6 (OFF)
-- power on
if (ar1.CaptureCardConfig_EthInit("192.168.30.30", "192.168.30.180", "12:34:56:78:90:10", 4096, 4098) == 0) then
